...
func NewDriver(version string, ctx *cli.Context) (*Driver, error) {
	...
	fmt.Printf("created vzlan driver\n")
	return d, nil
}
...
// CreateNetwork creates a new VZLAN network
...
// CreateEndpoint creates a new VZLAN Endpoint
...
// DeleteEndpoint deletes a VZLAN Endpoint
func (d *Driver) DeleteEndpoint(r *sdk.DeleteEndpointRequest) error {
	...
	log.Infof("Deleting the unused vzlan link [ %s ] from the removed container", link.Attrs().Name)
	if err := netlink.LinkDel(link); err != nil {
		log.Errorf("unable to delete the Vzlan link [ %s ] on leave: %s", link.Attrs().Name, err)
	}
	return nil
}

// EndpointInfo returns informatoin about a VZLAN endpoint
...
// Join creates a VZLAN interface to be moved to the container netns
func (d *Driver) Join(r *sdk.JoinRequest) (*sdk.JoinResponse, error) {
	...
	if getID.ifaceOpt == "" {
		return nil, fmt.Errorf("Required vzlan parent interface is missing, please recreate the network specifying the -o host_iface=ethX")
	}
	...
	if err := netlink.LinkAdd(mvlan); err != nil {
		log.Warnf("Failed to create the netlink link: [ %v ] with the "+
			"error: %s Note: a parent index cannot be link to both vzlan "+
			"and vzlan simultaneously. A new parent index is required", mvlan, err)
	...
	if err := netlink.LinkSetUp(mvlan); err != nil {
		log.Warnf("failed to enable the vzlan netlink link: [ %v ]", mvlan, err)
	}
...
// Leave removes a VZLAN Endpoint from a container
	...
	for _, n := range existingNets {
		...
		if n.Name != "" && n.Name != "none" && n.Name != "host" && n.Name != "bridge" {
			...
			for k, v := range n.Options {
				// Infer a vzlan network from required option
				if k == "host_iface" {
					nw.ifaceOpt = v
					log.Debugf("Existing vzlan network exists: [Name:%s, Cidr:%s, Gateway:%s, Master Iface:%s]",
